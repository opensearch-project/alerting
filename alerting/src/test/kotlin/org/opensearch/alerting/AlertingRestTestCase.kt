...
abstract class AlertingRestTestCase : ODFERestTestCase() {
    ...
    private fun createMonitorEntityWithBackendRoles(
        monitor: Monitor,
        rbacRoles: List<String>?,
    ): HttpEntity {
        if (rbacRoles == null) {
    ...
    private fun createMonitorV2EntityWithBackendRoles(
        monitorV2: MonitorV2,
        rbacRoles: List<String>?,
    ): HttpEntity {
        if (rbacRoles == null) {
    ...
    protected fun createMonitor(
        monitor: Monitor,
        refresh: Boolean = true,
    ): Monitor = createMonitorWithClient(client(), monitor, emptyList(), refresh)
    ...
    protected fun deleteMonitorV2(monitorV2Id: String): Response {
        val response =
            client().makeRequest(
                "DELETE",
                "$MONITOR_V2_BASE_URI/$monitorV2Id?refresh=true",
                emptyMap(),
            )
        assertEquals("Unable to delete a monitor", RestStatus.OK, response.restStatus())
     ...
     */
    protected fun createDestination(
        destination: Destination = getTestDestination(),
        refresh: Boolean = true,
    ): Destination {
    ...
    protected fun deleteDestination(
        destination: Destination = getTestDestination(),
        refresh: Boolean = true,
    ): Response {
        val response =
    ...
    protected fun updateDestination(
     ...
     * This method should only be used for checking legacy behavior/Notification migration scenarios.
     */
    protected fun createEmailAccount(
        emailAccount: EmailAccount = getTestEmailAccount(),
        refresh: Boolean = true,
    ): EmailAccount {
        // Create Alerting config index if it doesn't exist to avoid mapping issues with legacy destination indexing
        createAlertingConfigIndex()
    ...
    protected fun createRandomEmailAccountWithGivenName(
        refresh: Boolean = true,
        randomName: String,
    ): EmailAccount {
     ...
     * This method should only be used for checking legacy behavior/Notification migration scenarios.
     */
    protected fun createEmailGroup(
        emailGroup: EmailGroup = getTestEmailGroup(),
        refresh: Boolean = true,
    ): EmailGroup {
        // Create Alerting config index if it doesn't exist to avoid mapping issues with legacy destination indexing
        createAlertingConfigIndex()
    ...
    protected fun createRandomEmailGroupWithGivenName(
        refresh: Boolean = true,
        randomName: String,
    ): EmailGroup {
    ...
    @Suppress("UNCHECKED_CAST")
    protected fun getDestinations(dataMap: Map<String, Any> = emptyMap()): List<Map<String, Any>> = getDestinations(client(), dataMap)
    ...
    protected fun verifyIndexSchemaVersion(
        index: String,
        expectedVersion: Int,
    ) {
    ...
    protected fun createRandomMonitor(
    ...
    protected fun createRandomDocumentMonitor(
        refresh: Boolean = false,
        withMetadata: Boolean = false,
    ): Monitor {
    ...
    protected fun updateMonitorV2(
        ...
        val response =
            client().makeRequest(
                "PUT",
                "$MONITOR_V2_BASE_URI/${monitorV2.id}?refresh=$refresh",
                emptyMap(),
                monitorV2.toHttpEntity(),
            )
    ...
    protected fun acknowledgeAlerts(
        monitor: Monitor,
        vararg alerts: Alert,
    ): Response {
        val request =
    ...
    protected fun acknowledgeChainedAlerts(
        workflowId: String,
        vararg alertId: String,
    ): Response {
        val request =
            jsonBuilder()
                .startObject()
    ...
    protected fun getAlerts(
        dataMap: Map<String, Any> = emptyMap(),
        header: BasicHeader = BasicHeader(HttpHeaders.CONTENT_TYPE, "application/json"),
    ): Response = getAlerts(client(), dataMap, header)
    ...
    ): Response = executeMonitor(client(), monitorId, params)

    protected fun executeWorkflow(
        workflowId: String,
        params: Map<String, String> = mutableMapOf(),
    ): Response = executeWorkflow(client(), workflowId, params)
    ...
    protected fun executeMonitor(
        ...
        monitorId: String,
        params: Map<String, String> = mutableMapOf(),
    ): Response = client.makeRequest("POST", "$ALERTING_BASE_URI/$monitorId/_execute", params)

    protected fun executeWorkflow(
        client: RestClient,
        workflowId: String,
        params: Map<String, String> = mutableMapOf(),
    ): Response = client.makeRequest("POST", "$WORKFLOW_ALERTING_BASE_URI/$workflowId/_execute", params)

    protected fun executeMonitor(
        monitor: Monitor,
        params: Map<String, String> = mapOf(),
    ): Response = executeMonitor(client(), monitor, params)

    protected fun executeMonitor(
        client: RestClient,
        monitor: Monitor,
        params: Map<String, String> = mapOf(),
    ): Response = client.makeRequest("POST", "$ALERTING_BASE_URI/_execute", params, monitor.toHttpEntityWithUser())

    protected fun executeMonitorV2(
        monitorId: String,
        params: Map<String, String> = mutableMapOf(),
    ): Response = client().makeRequest("POST", "$MONITOR_V2_BASE_URI/$monitorId/_execute", params)
    ...
    protected fun indexDoc(
        ...
        doc: String,
        refresh: Boolean = true,
    ): Response = indexDoc(client(), index, id, doc, refresh)

    protected fun indexDocWithAdminClient(
        index: String,
        id: String,
        doc: String,
        refresh: Boolean = true,
    ): Response = indexDoc(adminClient(), index, id, doc, refresh)

    private fun indexDoc(
        client: RestClient,
        index: String,
        id: String,
        doc: String,
    ...
    protected fun createTestAlias(
        ...
        createTestAlias(alias = alias, indices = randomAliasIndices(alias, numOfAliasIndices, includeWriteIndex))
    ...
    protected fun createIndexAlias(
        alias: String,
        mappings: String?,
        setting: String? = "",
    ) {
    ...
    protected fun createComponentTemplateWithMappings(
    ...
    protected fun createComponentTemplateWithMappingsAndSettings(
        componentTemplateName: String,
        mappings: String?,
        setting: String?,
    ) {
    ...
    protected fun insertSampleTimeSerializedDataCurrentTime(
        index: String,
        data: List<String>,
    ) {
        data.forEachIndexed { i, value ->
    ...
    protected fun deleteDataWithDocIds(
        index: String,
        docIds: List<String>,
    ) {
        docIds.forEach {
...
//        createIndex(AlertIndices.FINDING_HISTORY_WRITE_INDEX, settings, mappingHack)
    ...
    fun scheduledJobMappings(): String = javaClass.classLoader.getResource("mappings/scheduled-jobs.json").readText()
    ...
    protected fun Monitor.toHttpEntity(): HttpEntity = StringEntity(toJsonString(), APPLICATION_JSON)
    ...
    protected fun Monitor.toHttpEntityWithUser(): HttpEntity = StringEntity(toJsonStringWithUser(), APPLICATION_JSON)
    ...
    protected fun MonitorV2.toHttpEntity(): HttpEntity = StringEntity(toJsonString(), APPLICATION_JSON)

    private fun MonitorV2.toJsonString(): String = shuffleXContent(toXContent(jsonBuilder(), ToXContent.EMPTY_PARAMS)).string()

    protected fun MonitorV2.toHttpEntityWithUser(): HttpEntity = StringEntity(toJsonStringWithUser(), APPLICATION_JSON)
    ...
    protected fun Destination.toHttpEntity(): HttpEntity = StringEntity(toJsonString(), APPLICATION_JSON)
    ...
    protected fun EmailAccount.toHttpEntity(): HttpEntity = StringEntity(toJsonString(), APPLICATION_JSON)
    ...
    protected fun EmailGroup.toHttpEntity(): HttpEntity = StringEntity(toJsonString(), APPLICATION_JSON)
    ...
    protected fun Alert.toHttpEntityWithUser(): HttpEntity = StringEntity(toJsonStringWithUser(), APPLICATION_JSON)
    ...
    fun getAlertingStats(
        metrics: String = "",
        alertingVersion: String? = null,
    ): Map<String, Any> {
            ...
            client().makeRequest(
                ...
                StringEntity(
                    XContentFactory
                        ...
                        .field("persistent")
                        .startObject()
                        .field(ScheduledJobSettings.SWEEPER_ENABLED.key, true)
                        .endObject()
                        .endObject()
                        .string(),
            ...
            client().makeRequest(
                ...
                StringEntity(
                    XContentFactory
                        ...
                        .field("persistent")
                        .startObject()
                        .field(ScheduledJobSettings.SWEEPER_ENABLED.key, false)
                        .endObject()
                        .endObject()
                        .string(),
                    ContentType.APPLICATION_JSON,
            ...
            client().makeRequest(
                ...
                StringEntity(
                    XContentFactory
                        ...
                        .field("persistent")
                        .startObject()
                        .field(AlertingSettings.FILTER_BY_BACKEND_ROLES.key, true)
                        .endObject()
                        .endObject()
                        .string(),
                    ContentType.APPLICATION_JSON,
            ...
            client().makeRequest(
                ...
                StringEntity(
                    jsonBuilder()
                        ...
                        .field("persistent")
                        .startObject()
                        .field(AlertingSettings.FILTER_BY_BACKEND_ROLES.key, false)
                        .endObject()
                        .endObject()
                        .string(),
                    ContentType.APPLICATION_JSON,
    ...
    fun createUserWithAttributes(
        ...
        val customAttributesString =
            customAttributes.entries.joinToString(prefix = "{", separator = ", ", postfix = "}") {
                "\"${it.key}\": \"${it.value}\""
            }
        var entity =
            """
    ...
    private fun createCustomIndexRole(
        name: String,
        index: String,
    ...
    fun createIndexRoleWithDocLevelSecurity(
        name: String,
        index: String,
        dlsQuery: String,
        clusterPermissions: String? = "",
    ...
    fun getClusterPermissionsFromCustomRole(clusterPermissions: String): String? = ROLE_TO_PERMISSION_MAPPING.get(clusterPermissions)
    ...
    protected fun createRandomWorkflow(
    ...
    private fun createWorkflowEntityWithBackendRoles(
        workflow: Workflow,
        rbacRoles: List<String>?,
    ): HttpEntity {
        if (rbacRoles == null) {
    ...
    protected fun createWorkflowWithClient(
        ...
        val response =
            client.makeRequest(
                "POST",
                "$WORKFLOW_ALERTING_BASE_URI?refresh=$refresh",
                emptyMap(),
                createWorkflowEntityWithBackendRoles(workflow, rbacRoles),
            )
        assertEquals("Unable to create a new monitor", RestStatus.CREATED, response.restStatus())
    ...
    protected fun createWorkflow(
        workflow: Workflow,
        refresh: Boolean = true,
    ): Workflow = createWorkflowWithClient(client(), workflow, emptyList(), refresh)

    protected fun Workflow.toHttpEntity(): HttpEntity = StringEntity(toJsonString(), APPLICATION_JSON)
    ...
    protected fun createAlertComment(
        alertId: String,
        content: String,
        client: RestClient,
    ): Comment {
        ...
        return Comment(
            ...
            lastUpdatedTime =
                if (comment["last_updated_time"] != null) {
    ...
    protected fun searchAlertComments(
    ...
    protected fun deleteAlertComment(
        commentId: String,
        client: RestClient,
    ): String {
        val deleteResponse =
    ...
    protected fun isMonitorScheduled(
        monitorId: String,
        alertingStatsResponse: Map<String, Any>,
    ): Boolean {
    ...
    protected fun isTriggered(
        pplMonitor: PPLSQLMonitor,
    ...
    protected fun getAlertV2HistoryDocCount(): Long {
        ...
        val response =
            adminClient().makeRequest(
                "POST",
                "${AlertV2Indices.ALERT_V2_HISTORY_ALL}/_search",
                emptyMap(),
