...
class PPLSQLMonitorRunnerIT : AlertingRestTestCase() {
    ...
    fun `test monitor execution timeout generates error alert`() {
        ...
        val getAlertsResponseMap = entityAsMap(getAlertsResponse)
        val alertsGenerated = numAlerts(getAlertsResponseMap) > 0
        val containsErrorAlert = containsErrorAlert(getAlertsResponseMap)
    ...
    fun `test running number of results condition and per result mode ppl monitor`() {
        ...
        val getAlertsResponse = getAlertV2s()
        val getAlertsResponseMap = entityAsMap(getAlertsResponse)
        val alertsGenerated = numAlerts(getAlertsResponseMap)
    ...
    fun `test execute api generated alert gets expired`() {
        ...
        val getAlertsResponsePreExpireMap = entityAsMap(getAlertsResponsePreExpire)
        val alertsGeneratedPreExpire = numAlerts(getAlertsResponsePreExpireMap) > 0
        ...
        val getAlertsResponsePostExpireMap = entityAsMap(getAlertsResponsePostExpire)
        val alertsGeneratedPostExpire = numAlerts(getAlertsResponsePostExpireMap) > 0
    ...
    fun `test scheduled job monitor execution gets throttled`() {
        ...
        val getAlertsResponsePreThrottleMap = entityAsMap(getAlertsResponsePreThrottle)
        val numAlertsPreThrottle = numAlerts(getAlertsResponsePreThrottleMap)
        ...
        val getAlertsResponsePostThrottleMap = entityAsMap(getAlertsResponsePostThrottled)
        val numAlertsPostThrottled = numAlerts(getAlertsResponsePostThrottleMap)
    ...
    fun `test manual monitor execution bypasses throttle`() {
        ...
        val getAlertsResponse = getAlertV2s()
        val getAlertsResponseMap = entityAsMap(getAlertsResponse)
        val numAlerts = numAlerts(getAlertsResponseMap)
        ...
        val getAlertsResponseAgainMap = entityAsMap(getAlertsAgainResponse)
        val numAlertsAgain = numAlerts(getAlertsResponseAgainMap)
