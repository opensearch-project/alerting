 ...
 */
class TransportIndexMonitorV2Action
    @Inject
    constructor(
        ...
        override fun doExecute(
            ...
            when (indexMonitorV2Request.monitorV2) {
                is PPLSQLMonitor -> {
                    validatePplSqlMonitorUserPermissionsAndQuery(
                        indexMonitorV2Request,
                        user,
                        object : ActionListener<Unit> { // validationListener
                            override fun onResponse(response: Unit) {
                                ...
                                client.threadPool().threadContext.stashContext().use {
                                    ...
                                    if (user == null) {
                                        indexMonitorV2Request.monitorV2 =
                                            pplSqlMonitor
        ...
        private fun validatePplSqlMonitor(
            pplSqlMonitor: PPLSQLMonitor,
            validationListener: ActionListener<Unit>,
        ): Boolean {
        ...
        private suspend fun checkPplQueryIndicesForTimestampField(
            ...
            try {
                ...
                validationListener.onFailure(
                    AlertingException.wrap(
                        IllegalArgumentException(
                            "failed to read query indices' fields when checking for timestamp field: $timestampField",
                            e,
                        ),
                    ),
                )
            ...
            if (user != null) {
                ...
                val rbacRoles =
                    if (indexMonitorRequest.rbacRoles == null) {
                        ...
                        indexMonitorRequest.rbacRoles.intersect(user.backendRoles).toSet()
                    } else {
                        indexMonitorRequest.rbacRoles
                    }
