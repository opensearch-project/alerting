...
class DocLevelMonitorQueries(private val client: Client, private val clusterService: ClusterService) {
    ...
    private suspend fun updateQueryIndexMappings(
        ...
        if (targetQueryIndex == null) {
            ...
            // This is due to a fact that _rollover API would maintain only single index under alias
            // if you don't add is_write_index setting when creating index initially
            targetQueryIndex = getWriteIndexNameForAlias(ScheduledJob.DOC_LEVEL_QUERIES_INDEX)
            if (targetQueryIndex == null) {
                val message = "Failed to get write index for queryIndex alias:${ScheduledJob.DOC_LEVEL_QUERIES_INDEX}"
                log.error(message)
        ...
        try {
            ...
            if (unwrappedException.message?.contains("Limit of total fields") == true) {
                try {
                    ...
                    updateMappingResponse = client.suspendUntil {
                        client.admin().indices().putMapping(updateMappingRequestRetry, it)
                    }
                } catch (e: Exception) {
    ...
    private suspend fun rolloverQueryIndex(monitor: Monitor): String? {
        val queryIndex = ScheduledJob.DOC_LEVEL_QUERIES_INDEX
        val queryIndexPattern = queryIndex + INDEX_PATTERN_SUFFIX
