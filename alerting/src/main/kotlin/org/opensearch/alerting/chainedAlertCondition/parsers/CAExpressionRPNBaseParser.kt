 ...
 */
abstract class CAExpressionRPNBaseParser(
    ...
    protected fun convertInfixToPostfix(expTokens: List<String>): ArrayList<ExpressionToken> {
        ...
        for (tokenString in expTokens) {
            ...
            when (val expToken = assignToken(tokenString)) {
                is CAExpressionToken -> outputExpTokens.add(expToken)
                is CAExpressionOperator -> {
                    when (expToken) {
                        CAExpressionOperator.PAR_LEFT -> expTokenStack.push(expToken)
    ...
    private fun assignToken(tokenString: String): ExpressionToken {
        ...
        if (tokenString.startsWith(CAExpressionConstant.ConstantType.MONITOR.ident))
        ...
        for (con in CAExpressionConstant.ConstantType.values()) {
            if (tokenString == con.ident) return CAExpressionConstant(con)
        }
