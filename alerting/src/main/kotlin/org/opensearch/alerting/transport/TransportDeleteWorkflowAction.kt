 ...
 */
class TransportDeleteWorkflowAction
    @Inject
    constructor(
        ...
        override fun doExecute(
            ...
            actionListener: ActionListener<DeleteWorkflowResponse>,
        ) {
            val transformedRequest =
                request as? DeleteWorkflowRequest
                    ?: recreateObject(request) { DeleteWorkflowRequest(it) }

            val user = readUserFromThreadContext(client)
            val deleteRequest =
                DeleteRequest(ScheduledJob.SCHEDULED_JOBS_INDEX, transformedRequest.workflowId)
        ...
        inner class DeleteWorkflowHandler(
                ...
                try {
                    ...
                    if (canDelete) {
                        ...
                        try {
                            ...
                            val deleteMonitorWorkflowMetadataResponse: BulkByScrollResponse =
                                client.suspendUntil {
             ...
             */
            private suspend fun tryDeletingMonitors(
             ...
             */
            private suspend fun getDeletableDelegates(
                workflowIdToBeDeleted: String,
                monitorIds: List<String>,
                user: User?,
            ): List<Monitor> {
                // Retrieve monitors belonging to another workflows
                val queryBuilder =
                    QueryBuilders.boolQuery().mustNot(QueryBuilders.termQuery("_id", workflowIdToBeDeleted)).filter(
