    ...
    constructor(
                ...
                if (monitor.triggers.isEmpty()) {
                    ...
                    if (monitor.shouldCreateSingleAlertForFindings == null || monitor.shouldCreateSingleAlertForFindings == false) {
                        monitor.triggers.forEach {
                            triggerResults[it.id] =
                                runForEachDocTrigger(
                                    monitorResult,
                        ...
                        monitor.triggers.forEach {
                            triggerResults[it.id] =
                                runForEachDocTriggerCreateSingleGroupedAlert(
                                    monitorResult,
                                    it as DocumentLevelTrigger,
                                    monitor,
                                    queryToDocIds,
        ...
        private suspend fun runForEachDocTriggerCreateSingleGroupedAlert(
            ...
            if (triggerResult.triggeredDocs.isNotEmpty()) {
                ...
                val alert =
                    alertService.composeDocLevelAlert(
                        findingIds!!,
                ...
                if (!dryrun && monitor.id != Monitor.NO_ID) {
                    ...
                    val actionExecutionResults =
                        actionResults.values.map { actionRunResult ->
            ...
            triggerFindingDocPairs.forEach {
                ...
                alertContexts.add(
                    AlertContext(
                        alert = alert,
                        associatedQueries =
                            alert.findingIds.flatMap { findingId ->
        ...
        private fun isFanOutTimeEnded(endTime: Instant): Boolean = Instant.now().isAfter(endTime)
        ...
        private suspend fun performPercolateQueryAndResetCounters(
            ...
            try {
                val percolateQueryResponseHits =
                    runPercolateQueryOnTransformedDocs(
                        transformedDocs,
                        monitor,
                        monitorMetadata,
                        concreteIndices,
            ...
            while (it.hasNext()) {
                ...
                if (entry.value is Map<*, *>) {
                    ...
                    conflictingFields.forEach { conflictingField ->
                        if (conflictingField == "$fieldNamePrefix.${entry.key}" ||
                            (fieldNamePrefix == "" && conflictingField == entry.key)
                        ) {
        ...
        private fun shouldPerformPercolateQueryAndFlushInMemoryDocs(numDocs: Int): Boolean =
            isInMemoryDocsSizeExceedingMemoryLimit(docsSizeOfBatchInBytes) ||
