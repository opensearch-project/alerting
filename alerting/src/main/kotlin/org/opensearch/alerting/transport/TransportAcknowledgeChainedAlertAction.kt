...
class TransportAcknowledgeChainedAlertAction
    @Inject
    constructor(
        ...
        override fun doExecute(
            ...
            val request =
                AcknowledgeChainedAlertRequest as? AcknowledgeChainedAlertRequest
        ...
        private suspend fun getWorkflow(workflowId: String): GetResponse =
            client.suspendUntil {
                client.get(GetRequest(ScheduledJob.SCHEDULED_JOBS_INDEX, workflowId), it)
            }
        ...
        inner class AcknowledgeHandler(
            ...
            private suspend fun getDataSources(monitorId: String): DataSources {
                val getResponse: GetResponse =
                    client.suspendUntil {
                        client.get(
                            GetRequest(ScheduledJob.SCHEDULED_JOBS_INDEX, monitorId),
                            it,
                        )
                    }
            ...
            private suspend fun onSearchResponse(
                ...
                response.hits.forEach { hit ->
                    ...
                    if (alert.state == Alert.State.ACTIVE) {
                        if (
                            ...
                            val updateRequest =
                                UpdateRequest(dataSources.alertsIndex, alert.id)
                                    .routing(request.workflowId)
                            ...
                            val copyRequest =
                                IndexRequest(alertsHistoryIndex)
                                    ...
                                    .source(
                                        alert
                                            .copy(state = Alert.State.ACKNOWLEDGED, acknowledgedTime = Instant.now())
                ...
                try {
                    val updateResponse: BulkResponse? =
                    ...
                    val copyResponse: BulkResponse? =
                        if (copyRequests.isNotEmpty()) {
                            client.suspendUntil {
            ...
            private suspend fun onBulkResponse(
                ...
                copyResponse?.items?.forEach { item ->
                    ...
                    if (item.isFailed) {
                        ...
                        val deleteRequest =
                            DeleteRequest(dataSources.alertsIndex, item.id)
                                .routing(request.workflowId)
