        ...
        private suspend fun onSearchResponse(response: SearchResponse, workflow: Workflow, dataSources: DataSources) {
            ...
            response.hits.forEach { hit ->
                ...
                if (alert.state == Alert.State.ACTIVE) {
                    ...
                    // non-empty workflowId and non-empty findingIds. For these alerts ACK means
                    // "in progress" — keep the document in the active index so the next execution
                    // can continue appending findings to the same alert. For all other alerts the
                    // existing copy-to-history semantics are preserved.
