    ...
    constructor(
        ...
        fun checkIndicesAndExecute(
            ...
            searchInputs.forEach {
                val inputIndices =
                    if (it.name() == SearchInput.SEARCH_FIELD) {
                        (it as SearchInput).indices
                    } else if (it.name() == DOC_LEVEL_INPUT_FIELD) {
                        (it as DocLevelMonitorInput).indices
                    } else {
                        (it as RemoteDocLevelMonitorInput).docLevelMonitorInput.indices
                    }
        ...
        inner class IndexMonitorHandler(
                ...
                if (request.method == RestRequest.Method.PUT) {
                    ...
                    val query =
                        QueryBuilders.boolQuery().filter(
                            QueryBuilders.termQuery("${Monitor.MONITOR_TYPE}.type", Monitor.MONITOR_TYPE),
                        )
            ...
            private suspend fun indexMonitor() {
                ...
                try {
                    ...
                    try {
                        // delete monitor if metadata creation fails, log the right error and re-throw the error to fail listener
