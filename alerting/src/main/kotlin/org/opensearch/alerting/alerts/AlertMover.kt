...
private val log = LogManager.getLogger(MonitorRunnerService::class.java)
        ...
        if (
            ...
            try {
                ...
                while (i < delegates.size && (getResponse == null || getResponse.isExists == false)) {
                    getResponse =
                        client.suspendUntil { client.get(GetRequest(ScheduledJob.SCHEDULED_JOBS_INDEX, delegates[0].monitorId), it) }
                    if (getResponse!!.isExists) {
                        val monitor =
                            ScheduledJobUtils.parseMonitorFromScheduledJobDocSource(monitorCtx.xContentRegistry!!, response = getResponse)
